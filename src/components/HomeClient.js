...
import * as modes from "../constants/config";

export default function HomeClient() {
  const products = [...Products];
  let mode = modes.MODE_CART;
  let checkout = [];
    ...
    <div className=" bg-white grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      <main className="flex flex-col gap-8 row-start-2 items-center sm:items-start">
        {products.map((product, index) => {
          return (
            <div className="flex flex-row" key={product.Name}>
              <button
                type="button"
              ...
              <button type="button" className="w-4/5 text-blue-700 px-6 py-3.5">
                {product.Name}
              </button>
              <button
                type="button"
      ...
      <footer className="row-start-3 flex gap-6 flex-wrap items-center justify-center">
        <button
          type="button"
          className=" footer-button px-8 text-green-700 hover:text-white border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:hover:bg-blue-500 dark:focus:ring-blue-800"
        >
          Checkout
        </button>
        {/* <a
        ...
        <div className="w-1/2">{checkout.length}</div>
      </footer>
    </div>
  );
}
